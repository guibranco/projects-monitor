...
RESULT=$(curl -s -o /dev/null -w "%{http_code}" --location "http://localhost:8000/api/v1/messages.php")
if [ "$RESULT" -ne 200 ]; then
    CONTENT=$(curl --location "http://localhost:8000/api/v1/messages.php")
    echo "::set-output name=content::$CONTENT"
    echo "::set-output name=error::true"
    exit 1
fi

RESULT=$(curl -s -o /dev/null -w "%{http_code}" --location "http://localhost:8000/api/v1/log-message.php")
if [ "$RESULT" -ne 401 ]; then
    CONTENT=$(curl --location "http://localhost:8000/api/v1/log-message.php")
